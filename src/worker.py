...
class DecodeWorker(Thread):
    ...
    def _open_video(self, reopen: bool = False):
        ...
        if isinstance(self.video_path, int):
            ...
            fourcc = (self.config.camera_fourcc or "MJPG").upper()[:4]
            ...
            if cap.isOpened():
                ...
                print(
                    f"Stream {self.stream_id}: {action} camera index {self.video_path} "
                    f"as {fourcc} {real_w}x{real_h}@{real_fps:.1f}fps"
                )
                return cap
